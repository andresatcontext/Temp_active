...
class classificationDataset(dataset):
    def __init__(self,
        ...
        super(classificationDataset, self).__init__( batchsize)
        ...
        with tf.name_scope("Classification_Dataset"):
            ...
            self.data_dict["image"] = data_tensors[0]
            self.data_dict["labels"] = data_tensors[1]
            self.data_dict["files"] = data_tensors[2]
            self.data_dict["nb_classes"] = self.nb_classes
            self.data_dict["classes_names"] = self.class_names
            
    def load_data_cifar(self, cifar10_data):
        
        self.nb_classes = len(cifar10_data.classes)
    ...
    def load_metadata(self, no_image_check):
        ...
        #############################################################################################
        # Check if the dataset comes from Datanet
        #############################################################################################
        ...
        if os.path.exists(os.path.join(model_path,"classes.json")):
            with open(os.path.join(model_path,"classes.json"), "r") as f:

                list_classes = json.load(f)
                for i in range(len(list_classes)):
                    list_classes[i] = str(list_classes[i])
