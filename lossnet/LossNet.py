...
class Change_loss_weights(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        if epoch == self.split_epoch-1:
            ...
            print('Previus weigths', backend.get_value(self.model.loss_weights['l_pred_w']), backend.get_value(self.model.loss_weights['l_pred_s']))
                
        if epoch<self.split_epoch-1:
            backend.set_value(self.weight_w, self.weight_lossnet_loss)
            backend.set_value(self.weight_s, 0)
        else:
            backend.set_value(self.weight_s, self.weight_lossnet_loss)
            backend.set_value(self.weight_w, 0)
